...
namespace TheFipster.Aviation.Modules.SimToolkitPro
{
    public class Importer
    {
        ...
        public void Read(string filepath)
        {
            ...
            foreach (var log in export.Logbook
                         .Where(x => x.Status.ToLower() == "completed")
                         .OrderBy(x => x.LocalId))
            {
                var landing = export.Landings.FirstOrDefault(x => x.FlightId == log.LocalId);
                var flight = new SimToolkitProFlight(log, landing);

                new JsonWriter<SimToolkitProFlight>().Write(flight, "SimToolkitPro", flight.Logbook.Dep, flight.Logbook.Arr);
            }
        }
    }
}